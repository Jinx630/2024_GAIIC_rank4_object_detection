...
def save_one_json(predn, jdict, path, class_map, dignoal=False):
...
def run_mlab(
    ...
    if weights:
        ...
        if 'mlab' in weights:
            detect_head = 2
        if 'prelatermlab' in weights:
            detect_head = 4
        if 'prelaterattmlab' in weights:
            detect_head = 5
        if 'prelatercrossmlab' in weights:
            detect_head = 6
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[1]:
            ...
            preds, train_out = model(img_rgb, img_ir, augment=augment) if compute_loss else (model(img_rgb, img_ir, augment=augment), None)

        # train_out 输出 2个list
        # 3
        # torch.Size([12, 69, 68, 84])
        # torch.Size([12, 69, 34, 42])
        # torch.Size([12, 69, 17, 21])
        # 3
        # torch.Size([12, 69, 68, 84])
        # torch.Size([12, 69, 34, 42])
        # torch.Size([12, 69, 17, 21])
        # preds 输出 2个list
        # torch.Size([12, 9, 7497])
        # torch.Size([12, 9, 7497])

        # Loss
        if compute_loss:
            preds = preds[detect_head]
            #train_out = train_out[1]
            #loss += compute_loss(train_out, targets)[1]  # box, obj, cls
        else:
            preds = preds[0][detect_head]
