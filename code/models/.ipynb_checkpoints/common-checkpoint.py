    ...
    def __init__(self, in_channels, out_channels):
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d):
                ...
                m.weight.data.normal_(0, math.sqrt(2.0 / n + 1e-8))
                m.bias.data.fill_(0)
...
class RepNCSPELAN4(nn.Module):
    ...
    def forward(self, x):
        ...
        # x.shape torch.Size([6, 128, 160, 160])
        # out.shape torch.Size([6, 256, 160, 160])
        return out
...
class DetectMultiBackend(nn.Module):
    ...
    def forward(self, im, im2, augment=False, visualize=False):
        ...
        if self.nhwc:
            ...
            im2 = im2.permute(0, 2, 3, 1).contiguous()
...
class CBAM(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16, pool_types=["avg", "max"], spatial=True):
        ...
        self.channel_attention = Channel_Attention(
            in_channels=in_channels, reduction_ratio=reduction_ratio, pool_types=pool_types
        )
