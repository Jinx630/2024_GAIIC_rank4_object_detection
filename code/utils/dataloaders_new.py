...
from utils.aug_utils import adjust_brightness_and_contrast, add_gaussian_noise, \
    BlurImage, rotate_and_crop_rgb_image, rotate_and_crop_tir_image, AugSaltNoise, AugEdge, \
    AugColor
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
           if self.hyp and self.hyp['custom_aug']:
                enhancement_decision = random.randint(1, 2)  # 随机决定执行哪一种增强
                if enhancement_decision == 1:
                    ...
                    aug_color = AugColor()
                    img_rgb = aug_color.forward(img_rgb)
                    img_ir = aug_color.forward(img_ir)
           else:
                # Augment colorspace
                augment_hsv(img_rgb, hgain=hyp["hsv_h"], sgain=hyp["hsv_s"], vgain=hyp["hsv_v"])
                augment_hsv(img_ir, hgain=hyp["hsv_h"], sgain=hyp["hsv_s"], vgain=hyp["hsv_v"])
        ...
        if self.augment:
            ...
            if self.hyp and self.hyp['custom_aug']:
                if random.random() < 0.1:
                    ...
                    aug_edge = AugEdge()
                    img_ir = aug_edge.forward(img_ir)
                if random.random() < 0.3:
                    ...
                    if enhancement_decision == 1:
                        ...
                        aug_salt_noise = AugSaltNoise()
                        img_rgb = aug_salt_noise.forward(img_rgb)
                        img_ir = aug_salt_noise.forward(img_ir)
