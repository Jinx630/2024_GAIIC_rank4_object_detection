...
class ComputeLoss:
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats[:3]], 2).split(
        ...
        # # con loss
        # label = torch.arange(feats[0].shape[0]).to(feats[0].device)

        # ce_loss = nn.CrossEntropyLoss(ignore_index=-100, reduction = 'mean')
        # pred_1,pred_2 = feats[-2],feats[-1]

        # pred_1_to_pred_2 =  pred_1 @ pred_2.t()
        # loss_ce = ce_loss(pred_1_to_pred_2, label)

        # pred_2_to_pred_1 =  pred_2 @ pred_1.t()
        # loss_ce += ce_loss(pred_2_to_pred_1, label)
