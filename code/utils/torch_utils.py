...
def smart_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
    ...
    bn = tuple(v for k, v in nn.__dict__.items() if 'Norm' in k and k is not 'InstanceNorm2d')  # normalization layers, i.e. BatchNorm2d()
    ...
    for v in model.modules():
        if hasattr(v, 'bias') and isinstance(v.bias, nn.Parameter):  # bias (no decay)
            g[2].append(v.bias)
        if hasattr(v, 'w1') and isinstance(v.w1, nn.Parameter):  # bias (no decay)
            g[2].append(v.w1)
        if hasattr(v, 'w2') and isinstance(v.w2, nn.Parameter):  # bias (no decay)
            g[2].append(v.w2)
        if hasattr(v, 'pos_emb_vis') and isinstance(v.pos_emb_vis, nn.Parameter):  # bias (no decay)
            g[2].append(v.pos_emb_vis)
        if hasattr(v, 'pos_emb_ir') and isinstance(v.pos_emb_ir, nn.Parameter):  # bias (no decay)
            g[2].append(v.pos_emb_ir)

        if hasattr(v, 'pos_emb') and isinstance(v.pos_emb, nn.Parameter):  # bias (no decay)
            g[2].append(v.pos_emb)
