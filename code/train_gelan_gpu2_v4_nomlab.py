...
import val_ir_rgb as validate  # for end-of-epoch mAP
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    is_coco = isinstance(test_path_ir, str) and test_path_ir.endswith('val2017.txt')  # COCO dataset
...
#         if 'yolov9' in weights:
#             new_csd = {}
#             for key in list(csd.keys()):
#                 if int(key.split('.')[1]) < 29:
#                     new_csd[key] = csd[key]
#                 elif 29 <= int(key.split('.')[1]) <= 35:
#                     new_csd[key] = csd[key]
...
#         csd = new_csd
        ...
        model_add_layers = {'c': [9, 12], 'e': [29, 32]}
        for key in list(state_dict.keys()):
            # if '29' in key:
                # continue
            if all(x not in key for x in exclude):
                # 复制 2份 backbone 的参数
                # try:
                    key_list = key.split('.')
                    suffix = '.' + '.'.join(key_list[2:])
                    if int(key_list[1]) < model_add_layers[opt.model_size][0]:
                        if state_dict[key].shape == model_state_dic[key].shape:
                            new_state_dict[key] = state_dict[key]
                        new_key = key[:6] + str(int(key_list[1]) + model_add_layers[opt.model_size][0]) + suffix
                        if state_dict[key].shape == model_state_dic[new_key].shape:
                            new_state_dict[new_key] = state_dict[key]
                    else:
                        new_key = key[:6] + str(int(key_list[1]) + model_add_layers[opt.model_size][1]) + suffix
                        if state_dict[key].shape == model_state_dic[new_key].shape:
                            new_state_dict[new_key] = state_dict[key]
        from collections import OrderedDict
        csd = OrderedDict(new_state_dict)

        model.load_state_dict(csd, strict=False)  # load

        LOGGER.info(f'Transferred {len(csd)}/{len(model.state_dict())} items from {weights}')  # report
        match_keys = list(csd.keys())
        match_keys = sorted(match_keys, key=lambda x: int(x.split('.')[1]))
        with open('./match_keys.json', 'w') as f:
            json.dump(match_keys, f, ensure_ascii=False, indent=4)
    else:
    ...
    # Trainloader
    # train_loader, dataset = create_dataloader(train_path_ir,
    #                                           imgsz,
    ...
    train_loader, dataset = create_dataloader_tir_rgb(train_path_rgb, train_path_ir,
                                                      imgsz,
                                                      batch_size // WORLD_SIZE,
                                                      gs,
                                                      single_cls,
    ...
    # Process 0
    if RANK in {-1, 0}:
        # val_loader = create_dataloader(test_path_ir,
        #                                imgsz,
        ...
        val_loader = create_dataloader_tir_rgb(test_path_rgb, test_path_ir,
                                               imgsz,
                                               batch_size // WORLD_SIZE * 2,
                                               gs,
                                               single_cls,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results = validate.run_layer(data_dict,
...
#                 if f is best:
#                     LOGGER.info(f'\nValidating {f}...')
#                     results, _, _ = validate.run_layer(
#                         data_dict,
#                         batch_size=batch_size // WORLD_SIZE * 2,
#                         imgsz=imgsz,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--hyp', type=str, default="code/data/hyps/hyp.scratch-high_copy.yaml", help='hyperparameters path')
    ...
    parser.add_argument('--model_size', type=str, default='e', help='yolo v9的模型大小 c or e')
    parser.add_argument('--custom_aug', type=bool ,default=True, help='是否使用自定义的数据增强')
    # Logger arguments
